  ...
  if (
    (acquiredCreditsCountSum >= requiredCreditsCount.min &&
      selectedCreditsCountMin >
        acquiredCreditsCountSum - requiredCreditsCount.max) ||
    (registeredCreditsCountSum >= requiredCreditsCount.min &&
      selectedCreditsCountMin >
        registeredCreditsCountSum - requiredCreditsCount.max)
  ) {
    yield selectedCourses;
  }
...
const pickRequirementWithFewestCombinations = (
  requirementAndGeneratorPairs: readonly {
...
function* constructAssignments(
  requirements: readonly RequirementWithCourses[],
  plan: Plan
): Generator<Plan, void, undefined> {
  ...
  const requirementAndGeneratorPairs: readonly {
    ...
    const registeredCourses = requirement.courses.filter(
      (course) =>
        plan.courseToRequirement.get(course) === undefined &&
        plan.courseToStatus.get(course) === RegistrationStatus.Registered
    );
    const acquiredCourses = requirement.courses.filter(
      (course) =>
        plan.courseToRequirement.get(course) === undefined &&
        plan.courseToStatus.get(course) === RegistrationStatus.Acquired
    );
  ...
  for (const combination of pickedCombinations) {
    ...
    const plans = constructAssignments(
      requirements.filter((requirement) => requirement !== pickedRequirement),
      plan0
    );
    yield* plans;
  }
...
function* findAssignments0(
  requirement: Requirements,
  plan: Plan
): Generator<readonly Plan[], void, undefined> {
  const requirements = requirement.getVisibleRequirements(
  ...
  for (const plan1 of constructAssignments(requirements, plan0)) {
    ...
    if (planAndCreditCountsPairs === undefined) {
      ...
      const acquired: {
        ...
        creditCounts.acquired >
          planAndCreditCountsPairs.acquired.creditCounts.acquired ||
        (creditCounts.acquired ===
          planAndCreditCountsPairs.acquired.creditCounts.acquired &&
          creditCounts.registered >
            planAndCreditCountsPairs.acquired.creditCounts.registered)
          ? {
              plan: plan1,
              creditCounts,
            }
      ...
      const registered: {
        ...
        creditCounts.registered >
          planAndCreditCountsPairs.registered.creditCounts.registered ||
        (creditCounts.registered ===
          planAndCreditCountsPairs.registered.creditCounts.registered &&
          creditCounts.acquired >
            planAndCreditCountsPairs.registered.creditCounts.acquired)
          ? {
              plan: plan1,
              creditCounts,
            }
      ...
      if (
        acquired !== planAndCreditCountsPairs.acquired ||
        registered !== planAndCreditCountsPairs.registered
      ) {
...
function* findAssignments(
  ...
  for (const selectionNameToOptionName of enumerateOptionsSelections(
    requirement,
    new Map()
  )) {
    let plans1: readonly Plan[] = plans0;
    for (const plans2 of findAssignments0(requirement, {
      ...plan,
      selectionNameToOptionName,
    })) {
